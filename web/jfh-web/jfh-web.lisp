...
(defun define-ps-with-html-macro ()
  (ps:ps
   (defun create-an-element (parent-element tag)
     (let ((new-element (ps:chain document (create-element tag))))
       (ps:chain parent-element (append-child new-element))
       new-element))
   (defun set-an-attribute (parent-element key value)
     (ps:chain parent-element (set-attribute key value)))
   (defun set-text-node (parent-element text)
     (let ((a-text-node (ps:chain document (create-text-node text))))
       (ps:chain parent-element (append-child a-text-node))))
              ...
              (let* ((expression (read-from-string expression-string))
                ...
                (if (event-attribute-p key)
                    (let ((event-attribute-key (subseq key 2)))
                      `(ps:chain ,parent-element (add-event-listener ,event-attribute-key (ps:chain ,function-name (bind null ,@parameters)) false)))
                            ...
                            #'(lambda (todo index)
                                (let ((checkbox-id (+ "todo-check" index))
                                  ...
                                  (with-html-elements
                                      (tr 
                                       (td
                                        (input (id . "(ps:chain checkbox-id (to-string))") (type . "checkbox") (onclick . "(update-todo (ps:chain index (to-string)))") (checked . "t"))
                                        (input (id . "test-check") (type . "button") (onclick . "(update-todo 123)"))
                                        (label (id . "(ps:chain label-id (to-string))") (html-for . "(ps:chain checkbox-id (to-string))") todo))
                                       (td "(+ 1 3)"))))
